...
namespace Lab7
{
    ...
    { 
        ...
        private void btnAnalyze_Click(object sender, EventArgs e)
        {
            ...
            int errCode;
            Study = (CWStudy)StudyMngr.CreateNewStudy(analyzName, (int)swsAnalysisStudyType_e.swsAnalysisStudyTypeStatic, (int)swsMeshType_e.swsMeshTypeSolid, out errCode);
            if (Study == null)
            {
            ...
            if (isRebuilded == true)
            {
                DispArray1 = new object[] { GetFaces((ModelDoc2)doc)[3] };
            }
            else
            {
                DispArray1 = new object[] { GetFaces((ModelDoc2)doc)[4] };
            }
            ...
            if (isRebuilded == true)
            {
                DispArray2 = new object[] { GetFaces((ModelDoc2)doc)[19] };
                DispArray3 = new object[] { GetFaces((ModelDoc2)doc)[18] };
            }
            else
            {
               DispArray2 = new object[]  { GetFaces((ModelDoc2)doc)[23] };
               DispArray3 = new object[] { GetFaces((ModelDoc2)doc)[22] };
            }
            ...
            StudyMngr.ActiveStudy = c;
            Study = StudyMngr.GetStudy(c);
        ...
        private void btReproject_Click(object sender, EventArgs e)
        {
            var ff = GetFaces(doc);
            isRebuilded = true;

            var ent = ff[11] as Entity;
            ent.Select(false);

            SketchManager sm = doc.SketchManager;
            FeatureManager fm = doc.FeatureManager;
            sm.InsertSketch(false);
            bool st = sm.SketchUseEdge2(false);
            var feature = featureExtrusion(0.01, false);

            var ent1 = ff[8] as Entity;
            ent1.Select(false);

            SketchManager sm1 = doc.SketchManager;
            FeatureManager fm1 = doc.FeatureManager;
            sm.InsertSketch(false);
            bool st1 = sm.SketchUseEdge2(false);
            var feature1 = featureExtrusion(0.025, false);

            var ent2 = ff[2] as Entity;
            ent2.Select(false);

            SketchManager sm2 = doc.SketchManager;
            FeatureManager fm2 = doc.FeatureManager;
            sm.InsertSketch(false);
            bool st2 = sm.SketchUseEdge2(false);
            var feature2 = featureExtrusion(0.01, false);
